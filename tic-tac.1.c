...
int player(char p){
  ...
  if ((int)p==(int)PLAYERFIRST)
  // print about player
  printf("[+] %s joueur [\x1B[48;5;%dm%c\x1B[0m] C'est ton tour\n[+] Insère ton nombre d'index pour placer ton coup",player1.name,21, p);
  else if ((int)p==(int)PLAYERSECOND)
  // print about player
  printf("[+] %s joueur [\x1B[48;5;%dm%c\x1B[0m] C'est ton tour\n[+] Insère ton nombre d'index pour placer ton coup",player2.name,88, p);
  ...
  // print va variable
  if ((int)p==(int)PLAYERFIRST)
  printf("[+]Joueur %s [\x1B[48;5;%dm%c\x1B[0m], Votre jeu est accepté : %d , Tour de jeu No. %d",player1.name,21,p, i, turn);

  else if ((int)p==(int)PLAYERSECOND)
  printf("[+]Joueur %s [\x1B[48;5;%dm%c\x1B[0m], Votre jeu est accepté : %d , Tour de jeu No. %d",player2.name,88,p, i, turn);
  // return value
  return ret;
};
...
int prompt(void)
{
  ...
  scanf("%d",&reponse);
  if (reponse ==1){printf("Connexion au serveur des meilleurs scores...\n");return 1;}
  else if (reponse ==2){printf("Ajout du score...\n");return 2;}
  else if (reponse ==3){printf("Nouvelle partie.\n");return 3;}
  else if (reponse ==4){printf("Merci, Au revoir\n");return 4;}
  else return 5;
}

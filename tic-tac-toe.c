...
Compilation :
gcc -Wall tic-tac-toe.c client2.c n-dial.c -o tic
Pour exécuter , tapez : ./tic
******************************************************* */
...
int appel (char *ip, char *port, char *order);
...
////fonction de traitement du nom des joueurs
int
detect_space (char *chaine)
{				////On enlève le retour à la ligne:
  char *pointeur = strtok (chaine, "\n");
  ////On termine la chaine au 10eme caractère
  if (strlen (chaine) > 10)
    (chaine[10] = 0);
  pointeur = chaine;
  while (*pointeur)
...
player (char p)
{
  ...
  //// ret permet de verifier si il y eu victoire
  return ret;
};
...
main (int ac, char *av[])
{
  ...
  while (1)
      ...
      while (win == 0)
	{
      ...
      ////Recyclage de la variable win
      ////qui sert a faire tourner la boucle du prompt
      while (win != 0)
	{
	  turn = prompt (av, win);
	  ////Si le choix de menu est 3, nouvelle partie avec les mêmes joueurs
	  if (turn == 3)
	    {
	      sprintf (s, ".........");
	      turn = 0;
...
int
appel (char *ip, char *port, char *order)
{
  int t;
  FILE *stream;
  t = dial (ip, port);
